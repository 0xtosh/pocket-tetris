...
static esp_err_t panel_st7735_init(esp_lcd_panel_t *panel)
{
    ...
    const st7735_lcd_init_cmd_t *init_cmds = st7735->init_cmds ? st7735->init_cmds : vendor_specific_init_default;
    uint16_t init_cmds_size = st7735->init_cmds ? st7735->init_cmds_size : sizeof(vendor_specific_init_default) / sizeof(st7735_lcd_init_cmd_t);
...
static esp_err_t panel_st7735_draw_bitmap(esp_lcd_panel_t *panel, int x_start, int y_start, int x_end, int y_end, const void *color_data)
{
    ...
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, LCD_CMD_CASET, (uint8_t[]){
        (x_start >> 8) & 0xFF, x_start & 0xFF, ((x_end - 1) >> 8) & 0xFF, (x_end - 1) & 0xFF}, 4), TAG, "send command failed");
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, LCD_CMD_RASET, (uint8_t[]){
        (y_start >> 8) & 0xFF, y_start & 0xFF, ((y_end - 1) >> 8) & 0xFF, (y_end - 1) & 0xFF}, 4), TAG, "send command failed");
...
{
    st7735_panel_t *st7735 = __containerof(panel, st7735_panel_t, base);
    return esp_lcd_panel_io_tx_param(st7735->io, invert_color_data ? LCD_CMD_INVON : LCD_CMD_INVOFF, NULL, 0);
}
...
static esp_err_t panel_st7735_mirror(esp_lcd_panel_t *panel, bool mirror_x, bool mirror_y)
{
    ...
    return esp_lcd_panel_io_tx_param(st7735->io, LCD_CMD_MADCTL, (uint8_t[]){st7735->madctl_val}, 1);
}
...
static esp_err_t panel_st7735_disp_on_off(esp_lcd_panel_t *panel, bool on_off)
{
    ...
    on_off = !on_off;
#endif
    return esp_lcd_panel_io_tx_param(st7735->io, on_off ? LCD_CMD_DISPON : LCD_CMD_DISPOFF, NULL, 0);
}
